# React Basics Assignment

## Description
This is a simple React application built to understand the basics of React including **components**, **props**, and **state**.

# live link https://rajit8674.github.io/gfg-Assignments/react-basics-assignment/dist/

## Features
- Reusable React components
- Props used to pass product data
- State used for counter increment and decrement
- Clean and centered UI using CSS Flexbox

## Tech Stack
- React (Vite)
- JavaScript
- CSS

## How to Run
```bash
npm install
npm run dev
